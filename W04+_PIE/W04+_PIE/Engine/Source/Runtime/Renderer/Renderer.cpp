...
void FRenderer::PrepareRender(UWorld* CurrentWorld)
{
    for (const auto actor : CurrentWorld->GetActors())
    {
        for (const auto comp : actor->GetComponents()) {
            ...
            if (ULightComponentBase* pLightComp = Cast<ULightComponentBase>(comp))
            {
