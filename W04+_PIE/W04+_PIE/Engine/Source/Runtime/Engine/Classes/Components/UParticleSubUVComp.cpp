...
void UParticleSubUVComp::TickComponent(float DeltaTime)
{
	...
	uint32 CellWidth = Sprite->width / CellsPerColumn;
	uint32 CellHeight = Sprite->height / CellsPerColumn;
	...
	float normalWidthOffset = float(CellWidth) / float(Sprite->width);
	float normalHeightOffset = float(CellHeight) / float(Sprite->height);
