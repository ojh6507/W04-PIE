...
void AController::ScreenToViewSpace(int screenX, int screenY, const FMatrix& viewMatrix, const FMatrix& projectionMatrix, FVector& pickPosition)
{
...
int AController::RayIntersectsObject(const FVector& pickPosition, USceneComponent* obj, float& hitDistance, int& intersectCount)
{
...
void AEditorController::Input(float DeltaTime)
{
...
bool AEditorController::PickGizmo(FVector& pickPosition)
{
...
void AEditorController::PickActor(const FVector& pickPosition)
{
...
void AEditorController::AddControlMode()
{
    cMode = static_cast<ControlMode>((cMode + 1) % CM_END);
}

void AEditorController::AddCoordiMode()
{
    cdMode = static_cast<CoordiMode>((cdMode + 1) % CDM_END);
}

void AEditorController::PickedObjControl()
{
...
void AEditorController::ControlRotation(USceneComponent* pObj, UGizmoBaseComponent* Gizmo, int32 deltaX, int32 deltaY)
{
    FVector cameraForward = GEngineLoop.GetLevelEditor()->GetActiveViewportClient()->ViewTransformPerspective.GetForwardVector();
    FVector cameraRight = GEngineLoop.GetLevelEditor()->GetActiveViewportClient()->ViewTransformPerspective.GetRightVector();
    FVector cameraUp = GEngineLoop.GetLevelEditor()->GetActiveViewportClient()->ViewTransformPerspective.GetUpVector();
...
void AEditorController::ControlTranslation(USceneComponent* pObj, UGizmoBaseComponent* Gizmo, int32 deltaX, int32 deltaY)
{
...
void AEditorController::ControlScale(USceneComponent* pObj, UGizmoBaseComponent* Gizmo, int32 deltaX, int32 deltaY)
{
    float DeltaX = static_cast<float>(deltaX);
    float DeltaY = static_cast<float>(deltaY);
    auto ActiveViewport = GetEngine().GetLevelEditor()->GetActiveViewportClient();
...
void APlayerController::Input(float DeltaTime)
{
    ImGuiIO& io = ImGui::GetIO();
    if (io.WantCaptureMouse) return;
    if (GetAsyncKeyState(VK_LBUTTON) & 0x8000)
    ...
    if (Player)
    {
        ...
        if (GetAsyncKeyState('W') & 0x8000)
        {
            PlayerLocation = PlayerLocation + Player->GetActorForwardVector() * DeltaTime;
        }
        if (GetAsyncKeyState('A') & 0x8000)
        {
            PlayerLocation = PlayerLocation - Player->GetActorRightVector() * DeltaTime;
        }
